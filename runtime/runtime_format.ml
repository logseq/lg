...
let format_with_string_ops ~text_length ~truncate source arguments =
  ...
  while !cursor < length do
    if source.[!cursor] <> '%' then (Buffer.add_char output source.[!cursor]; incr cursor)
    else (
      incr cursor;
      let start = !cursor in
      ...
      let conversion = source.[!cursor] in
      incr cursor;
      if not (String.contains "sSbBcCdoxXeEfgG%n" conversion) then invalid "unsupported conversion";
      ...
      let argument =
        if kind = '%' || kind = 'n' then Nil
          ...
          previous := Some selected; arguments.(selected))
      in

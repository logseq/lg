...
includes `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `exp`, `log`, `log10`,
`sqrt`, `to-radians`, and `to-degrees`, plus the upstream `E` and `PI`
constants. Remaining `cljs.math` functions stay explicitly deferred.

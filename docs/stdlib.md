...
call-site type tests and guard narrowing. `map?`, `vector?`, `coll?`, and
`associative?` have moved fully to the ClojureScript protocol model: their
source functions and inline specializations use `satisfies? IMap`,
`satisfies? IVector`, `satisfies? ICollection`, and `satisfies? IAssociative`,
with no predicate-specific name dispatch. Predicate
...
source-defined. After removing the `map?`, `vector?`, `coll?`, and
`associative?` name routes, the raw compiler-call inventory contains 236 names.

...
let split_qualified_symbol symbol =
  match String.index_opt symbol '/' with
  | None -> None
  | Some index ->
      let prefix = String.sub symbol 0 index in
      let name = String.sub symbol (index + 1) (String.length symbol - index - 1) in
      Some (prefix, name)

let add_alias_bindings env namespace alias =
      ...
      | Ok target, Ok value -> (
          ...
          | TRecord fields -> (
              ...
              | Some field when not (Types.equal field.ty value.ty) ->
                  Error.error
                    (Printf.sprintf "cannot assoc %s as %s because it is already %s"
                       keyword (source_name value.ty) (source_name field.ty))
              | Some _ ->
                  let values =
                    fields
              ...
              | None ->
                  ...
                  let values =
                    List.map
                      (fun (field, code) ->
                        if field.keyword = keyword then (field, value.code) else (field, code))
                      values
                  in
                  Ok { ty = TRecord fields; code = "<record>"; record_values = Some values })
...
let compile_top_level current_ns env next_type = function
  ...
  | FList (FSymbol "ns" :: FSymbol namespace :: clauses) -> (
      match parse_require_aliases clauses with
      | Error _ as err -> err
      | Ok aliases ->
          let env =
            aliases

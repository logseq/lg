              ...
              | Some declaration -> (
                  ...
                  let compile_method (method_id, signature) =
                    ...
                    let invocation_parameters =
                      match
                        (signature.Protocol_registry.param_tys, parameter_names)
                      with
                          ...
                          (fun result ->
                            Semantic_ir.Fun
                              ( List.map
                                  ...
                                  parameter_names,
                                result ))
                              (dynamic_unpack env signature.return_ty
                                 dynamic_result)))
                  in
...
and pack_dynamic_payload_impl env expected_dynamic argument =
  if Types.is_dynamic argument.ty then Ok argument.semantic_expr
    ...
    | TRef value_ty ->
        ...
        in
        Result.bind (pack_dynamic_value env expected_dynamic current)
          (fun packed_current ->
            Result.map
              (fun replacement ->
                Semantic_ir.Let
                  ...
                  if Types.equal expected_return actual_return then
                    ...
                    | TSeq expected_element, _ -> (
                        ...
                        | Ok (actual_element, sequence)
                          when Types.equal expected_element actual_element ->
                            Ok sequence
                        | Ok (actual_element, sequence) ->
                ...
                | Some field ->
                    Ok
                      ...
                      | ty when Types.is_dynamic ty -> (
                          ...
                          | _ ->
                              Error.error
                                (field_access ^ " expects a deftype value"))
                      | _ ->
                          Error.error (field_access ^ " expects a deftype value")
                      )
  ...
  and compile_concat scope env arg_forms =
    ...
    | Ok collections -> (
        ...
        | Ok ((first_ty, _) :: _ as sequences) ->
            if List.for_all (fun (ty, _) -> Types.equal first_ty ty) sequences
            then
              Ok
            ...
            else if List.exists (fun (ty, _) -> Types.is_dynamic ty) sequences
            then
              let dynamic_ty = Types.dynamic_constraint TUnknown in

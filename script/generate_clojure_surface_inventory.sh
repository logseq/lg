...
if test "$dispatch_count" -ne 316; then
  echo "compiler call dispatch changed: expected 316 names, found $dispatch_count" >&2
...
awk '
  BEGIN {
    ...
    split("apply assoc-in comp concat constantly cycle doall dorun drop drop-while every-pred ffirst filter filterv fnil get-in group-by interleave into juxt keep map map-indexed mapcat mapv max max-key merge min min-key next nfirst not-empty partial partition partition-all partition-by reduce reduce-kv reductions remove repeat repeatedly rest rseq run! select-keys some some-fn sort sort-by take take-nth take-while update-in vals vec", xs)

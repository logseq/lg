...
if test "$dispatch_count" -ne 326; then
  echo "compiler call dispatch changed: expected 326 names, found $dispatch_count" >&2
...
awk '
  BEGIN {
    ...
    split("apply assoc-in boolean comp concat constantly cycle dedupe distinct doall dorun drop drop-while every-pred every? ffirst filter filterv fnil fnext get-in group-by interleave interpose into juxt keep map map-indexed mapcat mapv max max-key merge min min-key next nfirst nnext not-empty nthrest partial partition partition-all partition-by reduce reduce-kv reductions remove repeat repeatedly rest reverse rseq run! select-keys some some-fn sort sort-by take take-nth take-while update-in vals vec zipmap", xs)

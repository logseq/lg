...
(reset-meta! fixture-watch-reference {:fixture :reset})
(alter-meta! fixture-watch-reference identity)
(zip/root nil)
...
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/object-array" && $3 == "source" && $4 == "source-port-preserves-upstream-size-seqable-fill-and-seq-padding-arities-through-typed-source-protocols-and-an-explicit-static-overload-interface-with-size-only-elements-remaining-polymorphic-static-array-values-and-only-using-the-existing-printer-boundary-when-printed-before-the-element-type-is-constrained" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*flush-on-newline*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-newline-flush-switch-through-the-static-output-line-primitive" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-newline*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-trailing-newline-output-switch-for-println-and-prn-through-the-static-output-primitive" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-readably*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-readable-versus-display-printer-selection-for-pr-str-pr-str-star-pr-prn-and-prn-str-through-static-printer-witnesses" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-meta*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-metadata-printing-switch-as-a-static-bool-ref-while-metadata-rendering-remains-explicitly-unsupported" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-dup*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-duplicate-printing-switch-as-a-static-bool-ref-while-duplicate-constructor-rendering-remains-explicitly-unsupported" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-namespace-maps*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-boolean-namespace-map-lifting-switch-as-a-static-bool-ref-while-prefix-map-rendering-remains-explicitly-unsupported" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-length*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-nil-or-integer-print-limit-as-a-typed-option-int-ref-threaded-through-static-readable-printer-witnesses" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/*print-level*" && $3 == "source" && $4 == "source-dynamic-var-preserves-the-upstream-nil-or-integer-recursive-print-depth-limit-through-static-readable-printer-witnesses" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/unsafe-bit-and" && $3 == "source" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/with-precision" && $3 == "source" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/vec-lite" && $3 == "source" && $4 == "source-public-wrapper-preserves-cljs-seqable-to-vector-realization-through-the-existing-static-vec-boundary" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/eduction" && $3 == "source" && $4 == "source-macro-expands-the-upstream-xform-star-and-final-collection-shape-to-the-existing-typed-eduction-constructor-while-first-class-variadic-eduction-values-remain-unrepresentable" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/memoize" && $3 == "source" && $4 == "source-macro-expands-direct-calls-to-a-private-typed-memoize-abi-for-zero-through-three-fixed-arity-functions-using-static-cache-keys-while-first-class-variadic-and-heterogeneous-argument-tuple-memoize-remains-unrepresentable" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/NaN?" && $3 == "source" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && $2 == "clojure.core/uuid?" && $3 == "source" && $4 == "source-public-function-matches-cljs-iuuid-predicate-with-a-first-class-nominal-uuid-signature-and-inline-static-specialization-that-recognizes-present-option-uuid-results-from-parse-uuid-and-distinguishes-ordinary-strings-without-dynamic-unpacking" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && ($2 == "clojure.core/add-watch" || $2 == "clojure.core/remove-watch" || $2 == "clojure.core/get-validator" || $2 == "clojure.core/set-validator!" || $2 == "clojure.core/reset-meta!" || $2 == "clojure.core/alter-meta!") && $3 == "source" {found++} END {exit found != 6}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && ($2 == "clojure.core/print-map" || $2 == "clojure.core/print-meta?" || $2 == "clojure.core/print-prefix-map") && $3 == "source" {found++} END {exit found != 3}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && ($2 == "clojure.core/-prefer-method" || $2 == "clojure.core/-prefers") && $3 == "source" && $4 == "runtime-multifn-boundary-provides-preference-table-mutation-and-introspection-through-a-documented-dynamic-boundary" {found++} END {exit found != 2}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "namespace" && $2 == "cljs.test" && $3 == "source" && $4 != "" {found=1} END {exit !found}' "$tmp/manifest-status.tsv"
awk -F '\t' '$1 == "definition" && index($2, "cljs.test/") == 1 && $3 == "blocked-static-typing" {print; found=1} END {exit found}' "$tmp/manifest-status.tsv"
...
awk -F '\t' '$1 == "definition" && $2 == "cljs.test/update-current-env!" && $3 == "source" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && $2 == "ex-data" {found=1} END {exit found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_ex-data" && $3 == "typed-primitive" && $4 == "documented-exception-info-data-dynamic-boundary" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && $2 == "re-find" {found=1} END {exit found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_re-find" && $3 == "typed-primitive" && $4 == "documented-regex-match-dynamic-boundary-with-static-optional-result-specialization" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && ($2 == "*exec-tap-fn*" || $2 == "add-tap" || $2 == "remove-tap" || $2 == "tap>") {found=1} END {exit found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_exec-tap-fn" && $3 == "typed-primitive" && $4 == "documented-tap-executor-callback-dynamic-boundary" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && ($2 == "__lg_add-tap" || $2 == "__lg_remove-tap") && $3 == "typed-primitive" && $4 == "documented-tap-registry-callback-dynamic-boundary" {found++} END {exit found != 2}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_tap" && $3 == "typed-primitive" && $4 == "documented-tap-value-dynamic-boundary" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_bound-fn" && $3 == "typed-primitive" && $4 == "typed-dynamic-var-binding-capture-and-function-wrapper-specialization-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && ($2 == "add-watch" || $2 == "remove-watch" || $2 == "get-validator" || $2 == "set-validator!" || $2 == "reset-meta!" || $2 == "alter-meta!") {found=1} END {exit found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_reset-meta!" && $3 == "typed-primitive" && $4 == "typed-reference-closed-edn-metadata-reset-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_print_output_line" && $3 == "typed-primitive" && $4 == "typed-static-string-output-with-boolean-newline-and-flush-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_print_values" && $3 == "typed-primitive" && $4 == "typed-display-values-to-bound-writer-or-standard-output-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_render_readable_values_with_opts" && $3 == "typed-primitive" && $4 == "typed-homogeneous-readable-printer-witness-sequence-rendering-with-print-length-options-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && ($2 == "print-map" || $2 == "print-meta?" || $2 == "print-prefix-map") {found=1} END {exit found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_print-map" && $3 == "typed-primitive" && $4 == "typed-static-map-printer-with-independent-key-value-witnesses-and-namespace-lifting" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_print-prefix-map" && $3 == "typed-primitive" && $4 == "typed-static-prefix-map-printer-with-independent-key-value-witnesses" {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "compiler-call" && $2 == "__lg_print-meta?" && $3 == "typed-primitive" && $4 == "typed-closed-edn-metadata-presence-and-meta-option-primitive" {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "logseq-namespace-status" && $2 == "cljs.pprint" && $3 == "source-aggregate" && $4 == 1 {found=1} END {exit !found}' "$tmp/inventory.tsv"
awk -F '\t' '$1 == "logseq-namespace-status" && $2 == "cljs.test" && $3 == "source-aggregate" && $4 == 1 {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "namespace-var" && ($2 == "cljs.test/assert-expr" || $2 == "cljs.test/update-current-env!") && $3 == "source" {found++} END {exit found != 2}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "logseq-core-var-status" && $2 == "clojure.core/re-seq" && $3 == "source-aggregate" && $4 > 0 {found=1} END {exit !found}' "$tmp/inventory.tsv"
...
awk -F '\t' '$1 == "logseq-qualified-var-status" && $2 == "cljs.spec.alpha/valid?" && $3 == "out-of-scope" && $4 == 1 && $5 == "spec-is-explicitly-excluded-from-the-lg-stdlib-port" {found=1} END {exit !found}' "$tmp/inventory.tsv"

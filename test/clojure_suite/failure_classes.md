...
| namespaces compiled on both native and Melange | 177 |
| namespaces failed on both native and Melange | 51 |
...
| --- | ---: | --- |
| `static-typing-or-closed-domain-boundary` | 99 | Split into intentional LG static errors and closed-domain boundaries. Do not weaken ordinary values or collections to dynamic. |
...
| `heterogeneous-collection-needs-closed-domain` | 20 | Add explicit closed domains only where the heterogeneous shape is part of a supported API; do not erase ordinary collections to dynamic. |
...
| `design-closed-domain-or-narrow-runtime-boundary` | 22 | Heterogeneous values and open event payloads need explicit closed domains or a documented minimal boundary. |
...
The 112 compile failures are not 112 independent core defects. The current

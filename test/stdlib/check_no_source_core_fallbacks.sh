...
for file in src/call_elaborator.ml src/core_sequence_transform.ml src/type_inference.ml; do
  if grep -F '"doall"' "$root/$file" >/dev/null; then
    echo "clojure.core/doall is still compiler-dispatched in $file" >&2
    exit 1
  fi
done

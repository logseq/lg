...
for name in identity complement boolean reduced ensure-reduced reset-vals! subs int-to-string-radix any? range shuffle alength acopy aslice aconcat array-to-seq array-to-rseq amap asort! quot rem mod unchecked-add unchecked-add-int unchecked-subtract unchecked-subtract-int unchecked-multiply unchecked-multiply-int unchecked-divide-int unchecked-remainder-int unchecked-inc unchecked-inc-int unchecked-dec unchecked-dec-int unchecked-negate unchecked-negate-int rand-int rand-nth bit-shift-right-zero-fill bit-and-not unsigned-bit-shift-right bit-count second last even? odd? every? ffirst fnext nfirst nnext not-any? not-every? split-at split-with nthnext nthrest bounded-count butlast take-last drop-last reverse interpose dedupe distinct zipmap comparator frequencies update-vals update-keys hash-combine max-key min-key constantly vec random-uuid parse-uuid system-time parse-long parse-double merge-with NaN? infinite? keyword-identical? symbol-identical? hash-long special-symbol?; do
  for file in \

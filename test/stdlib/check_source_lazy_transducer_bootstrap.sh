...
for name in filter remove take drop take-while drop-while map-indexed keep keep-indexed mapcat cat halt-when transduce sequence repeatedly take-nth random-sample partition-all partitionv-all repeat cycle; do
...
if rg -n 'apply_transducer|\| "(filter|remove|take|drop|take-while|drop-while|map-indexed|keep|keep-indexed|mapcat|cat|halt-when|transduce|sequence|repeatedly|take-nth|random-sample|partition-all|partitionv-all|repeat|cycle)" -> compile_' \

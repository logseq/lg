...
for name in filter remove take drop take-while drop-while map-indexed keep keep-indexed mapcat cat halt-when transduce sequence repeatedly reductions take-nth random-sample partition partition-all partitionv-all partition-by repeat cycle filterv dorun run! group-by sort sort-by; do
...
if rg -n 'apply_transducer|\| "(filter|remove|take|drop|take-while|drop-while|map-indexed|keep|keep-indexed|mapcat|cat|halt-when|transduce|sequence|repeatedly|reductions|take-nth|random-sample|partition|partition-all|partitionv-all|partition-by|repeat|cycle|filterv|dorun|run!|group-by|sort|sort-by)" -> compile_' \

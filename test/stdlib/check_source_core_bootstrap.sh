...
for name in comment doto when-first while if-not when when-not cond and or if-let when-let if-some when-some divide unchecked-max unchecked-min mask bitpos caching-hash '->' '->>' 'as->' 'cond->' 'cond->>' 'some->' 'some->>'; do

...
for name in comment doto when-first while if-not when when-not cond and or if-let when-let if-some when-some unchecked-max unchecked-min mask bitpos '->' '->>' 'as->' 'cond->' 'cond->>' 'some->' 'some->>'; do

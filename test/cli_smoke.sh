...
"$cli" --run-from "$stdlib_state" "$stdlib_implementation" "$package_source" > "$package_stdout"
...
grep -q 'continued_value.*magnitude_plus_two' "$partial_output"
...
"$cli" --run-files-from "$stdlib_state" "$stdlib_implementation" "$math_source" "$main_source" > "$multi_stdout"
[ "$(cat "$multi_stdout")" = "42" ]
...
printf '%s\n' \
  '(module LspMath' \
  '  (defn magnitude-plus-two [x] x))' > "$lsp_math_source"
...

{
  ...
  send_lsp_message "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"file://$main_source\",\"languageId\":\"lg\",\"version\":1,\"text\":\"(def result (LspMath/magnitude-plus-two 40))\\n\"}}}"
  ...
  send_lsp_message "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didChange\",\"params\":{\"textDocument\":{\"uri\":\"file://$lsp_math_source\",\"version\":2},\"contentChanges\":[{\"text\":\"(module LspMath (def value 1))\\n\"}]}}"
...
grep -q "\"id\":13,\"result\":{\"uri\":\"file://$lsp_math_source\"" "$lsp_output"
grep -q "\"id\":14,\"result\":.*\"uri\":\"file://$lsp_math_source\"" "$lsp_output"
...
grep -q "\"uri\":\"file://$lsp_math_source\"" "$lsp_output"

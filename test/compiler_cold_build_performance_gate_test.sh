...
if ! /usr/bin/time -p \
  ...
  dune build test/datascript_conn_native_base.ml \
...
if [ -z "$elapsed" ]; then
  echo "cold code build gate did not capture elapsed time" >&2
  cat "$timing_file" >&2
  exit 1
fi
...
awk -v elapsed="$elapsed" -v limit="$limit_seconds" 'BEGIN {
  if (elapsed > limit) {
    printf "cold code build exceeded limit: %.2fs > %.2fs\n", elapsed, limit > "/dev/stderr"
    exit 1
  }
  printf "cold code build: %.2fs (limit %.2fs)\n", elapsed, limit
}'

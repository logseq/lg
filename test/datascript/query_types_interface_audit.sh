...
interface_file="$workspace_root/test/datascript/lg/query_types.lgi"

if [[ ! -f "$interface_file" ]]; then
  echo "missing DataScript query-types interface: test/datascript/lg/query_types.lgi" >&2
  exit 1
fi
...
for manifest in "$workspace_root/test/dune" "$workspace_root/test/datascript_conn_tests.inc" "$workspace_root/test/compiler_tests.ml"; do
  ...
  interface_count=$(rg -c 'datascript/lg/query_types\.lgi' "$manifest" || true)

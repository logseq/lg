...
(defrecord MultivalAttrFrame [^:transient-vector acc ^datascript.pull-parser/PullAttr attr datoms]
  IFrame
  (-run [this context]
    (loop [acc acc
           datoms datoms]
      (util/cond+
        :let [^datascript.db/Datom datom (first-seq datoms)]
...
(defrecord MultivalRefAttrFrame [seen recursion-limits ^:transient-vector acc pattern ^datascript.pull-parser/PullAttr attr datoms]
  IFrame
  (-merge [this result]
    (let [value (.-value ^ResultFrame result)]
      (MultivalRefAttrFrame.
        seen
        recursion-limits
        (if (nil? value) acc (conj! acc value))
        pattern
        attr
        (next-seq datoms))))
...
(defrecord ReverseAttrsFrame [seen recursion-limits ^:transient-map acc pattern ^datascript.pull-parser/PullAttr attr attrs id]
  IFrame
  (-merge [this result]
    (let [value ((.-xform attr) (.-value ^ResultFrame result))]
      (ReverseAttrsFrame.
        seen
        recursion-limits
        (if (nil? value) acc (assoc! acc (.-as attr) value))
        pattern
        (first-seq attrs)
        (next-seq attrs)
        id)))
    ...
    (loop [acc   acc
      ...
      (util/cond+
        ...
        :let [name   (.-name attr)
              ...
              datoms (if (instance? datascript.db/DB db)
                       (set/slice (.-avet ^datascript.db/DB db) (db/datom db/e0 name id db/tx0) (db/datom db/emax name id db/txmax))
        ...
        [(ReverseAttrsFrame. seen recursion-limits acc pattern attr attrs id)
         (ref-frame context seen recursion-limits pattern attr (.-e ^datascript.db/Datom (first-seq datoms)))]
...
(defrecord AttrsFrame [seen recursion-limits ^:transient-map acc ^datascript.pull-parser/PullPattern pattern ^datascript.pull-parser/PullAttr attr attrs datoms id]
  IFrame
  (-merge [this result]
    (let [value ((.-xform attr) (.-value ^ResultFrame result))]
      (AttrsFrame.
...
(defn- auto-expanding? [^datascript.pull-parser/PullAttr attr]
  (or
    (.-recursive? attr)
    (and
      (.-component? attr)
      (.-wildcard? ^datascript.pull-parser/PullPattern (.-pattern attr)))))

(defn ref-frame [context seen recursion-limits pattern ^datascript.pull-parser/PullAttr attr id]
  (util/cond+
...
(defn attrs-frame [^Context context seen recursion-limits ^datascript.pull-parser/PullPattern pattern id]
  (let [db (.-db context)
        datoms (util/cond+
                 (and (.-wildcard? pattern) (instance? datascript.db/DB db))
                 (set/slice (.-eavt ^datascript.db/DB db) (db/datom id nil nil db/tx0) (db/datom id nil nil db/txmax))
                 ...
                 :let [from (.-name ^datascript.pull-parser/PullAttr (.-first-attr pattern))
                       to   (.-name ^datascript.pull-parser/PullAttr (.-last-attr pattern))]
                 
                 (instance? datascript.db/DB db)
                 (set/slice (.-eavt ^datascript.db/DB db) (db/datom id from nil db/tx0) (db/datom id to nil db/txmax))
...
(defn pull-impl [parsed-opts id]
  (let [{^Context context :context
         ^datascript.pull-parser/PullPattern pattern :pattern} parsed-opts]

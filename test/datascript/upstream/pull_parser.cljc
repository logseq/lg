...
(defn check [^boolean cond ^String expected ^:dynamic fragment]
  (when-not cond
...
(defn- conj-attr ^PullPattern [^PullPattern pull-pattern ^PullAttr pull-attr]
  (let [reverse? (:reverse? pull-attr)
        attrs    (if reverse?
                   (.-reverse-attrs pull-pattern)
                   (.-attrs pull-pattern))]
    (if-some [idx (index-of #(= (:as %) (:as pull-attr)) attrs)]
      (if reverse?
        (update pull-pattern :reverse-attrs assoc idx pull-attr)
        (update pull-pattern :attrs assoc idx pull-attr))
      (if reverse?
        (update pull-pattern :reverse-attrs conj pull-attr)
        (update pull-pattern :attrs conj pull-attr)))))

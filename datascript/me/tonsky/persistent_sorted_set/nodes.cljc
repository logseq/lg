...
(defn make-storage [restore accessed store delete]
  (record storage (restore restore) (accessed accessed) (store store) (delete delete)))

(defn- make-tree [keys children addresses address dirty]
  (record tree (keys keys) (children children) (_addresses addresses) (_address (volatile! address)) (_dirty (volatile! dirty))))
...
(defn node-children [node]
  (arrays/amap
    (fn [child]
      (match child
        (Some loaded) loaded
        None (Stdlib.failwith "persistent sorted set child is not loaded")))
    (:children node)))
...
(defn new-leaf [keys]
  (make-tree
    keys (arrays/empty-array) (arrays/empty-array) nil true))

(defn- new-leaf-address [keys address]
  (make-tree
    keys (arrays/empty-array) (arrays/empty-array) address true))

(defn new-restored-leaf [keys address]
  (make-tree
    keys (arrays/empty-array) (arrays/empty-array) (Some address) false))
...
(defn- new-node-state [keys children addresses address]
  (make-tree keys children addresses address true))
    ...
    (if-some [child (arrays/aget children idx)]
      (do
        (if-some [storage-value storage]
          (if-some [address (arrays/aget addresses idx)]
      ...
      (match storage
        ...
        (match (arrays/aget addresses idx)
          (Some address)
          (let [restore (:restore storage-value)]
            (match
              (restore address)
              (Some child)
              (do
        ...
        (cond
          ...
          (let [middle (arrays/half (inc keys-length))]
            (if (> idx middle)
              ...
              (arrays/array
                (new-leaf-address
                  (ops/cut-n-splice keys 0 middle idx idx (arrays/array key))
                  address)
                (new-leaf (arrays/aslice keys middle keys-length)))))
          :else
          (arrays/array
            (new-leaf-address
              (ops/splice keys idx idx (arrays/array key)) address))))
        ...
        (if-some [child-nodes (node-conj child cmp key storage)]
          (let [new-keys
            ...
            (if (<= (arrays/alength new-children) max-len)
              (arrays/array
                (new-node-state new-keys new-children new-addresses address))
    ...
    (if (= 0 (arrays/alength children))
      (let [idx (search/lookup-exact cmp keys key)]
        (if (= -1 idx)
          ...
          (rotate
            (new-leaf-address
              (ops/splice keys idx (inc idx) (arrays/empty-array))
              (deref (:_address node)))

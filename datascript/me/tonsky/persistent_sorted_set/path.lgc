...
(def max-safe-path #?(:native 2147483648 :melange 2147483648.0))
...
(def empty-path #?(:native 0 :melange 0.0))

(defn path-get
  #?(:native [^:int path ^:int level]
     :melange [^:float path ^:int level])
  #?(:native
...
(defn path-set
  #?(:native [^:int path ^:int level ^:int idx]
     :melange [^:float path ^:int level ^:int idx])
  #?(:native
...
(defn path-inc #?(:native [^:int path] :melange [^:float path])
  #?(:native (inc path) :melange (Float.add path 1.0)))

(defn path-dec #?(:native [^:int path] :melange [^:float path])
  #?(:native (dec path) :melange (Float.sub path 1.0)))

(defn path-cmp
  #?(:native [^:int path1 ^:int path2]
     :melange [^:float path1 ^:float path2])
  #?(:native (- path1 path2) :melange (Float.sub path1 path2)))
...
(defn path-str #?(:native [^:int path] :melange [^:float path])
  #?(:native
     (loop [result []
            path path]
       (if (= path 0)
